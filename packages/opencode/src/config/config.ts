...
export namespace Config {
    ...
    export const Keybinds = z
        .object({
            ...
            input_visual_line_end: z
                .string()
                .optional()
                .default("alt+e")
                .describe("Move to end of visual line in input"),
            ...
            input_delete_to_line_end: z
                .string()
                .optional()
                .default("ctrl+k")
                .describe("Delete to end of line in input"),
    ...
    export const global = lazy(async () => {
        ...
        if (isMongoEnabled()) {
            ...
            const mongoConfig = await MongoStorage.configGet().catch(() => null)
            if (mongoConfig) {
                const parsed = Info.safeParse(mongoConfig)
                if (parsed.success) {
                    log.info("Loaded config from MongoDB")
                    return parsed.data
                }
                log.warn("Invalid config in MongoDB, using defaults")
            }
            // Return minimal default config if MongoDB config not found
            return { $schema: "https://opencode.ai/config.json" } as Info
        }
