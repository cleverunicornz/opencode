...
export namespace Storage {
    ...
    // Check if MongoDB is enabled at module load
    ...
    const state = lazy(async () => {
        ...
        if (useMongoDb) {
            ...
            return { dir: "", useMongo: true as const }
        }
    ...
    export async function read<T>(key: string[]) {
        ...
        if (s.useMongo) {
            return withErrorHandling(async () => {
                return MongoStorage.read<T>(key)
            })
        }
        const target = path.join(s.dir, ...key) + ".json"
        return withErrorHandling(async () => {
    ...
    export async function update<T>(key: string[], fn: (draft: T) => void) {
        ...
        if (s.useMongo) {
            return withErrorHandling(async () => {
                return MongoStorage.update<T>(key, fn)
            })
        }
        const target = path.join(s.dir, ...key) + ".json"
        return withErrorHandling(async () => {
    ...
    export async function write<T>(key: string[], content: T) {
        const s = await state()
        if (s.useMongo) {
            return MongoStorage.write(key, content)
        }
        const target = path.join(s.dir, ...key) + ".json"
        return withErrorHandling(async () => {
            using _ = await Lock.write(target)
